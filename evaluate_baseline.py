...
# 0.114 is resolution in meter，每个像素对应0.114米的距离
...
def generate_distances(data_loader, order_list, delta_list):
    ...
    for i in range(data_loader.test_data_size):
        #全景图像真实的GPS位置，形式为（x，y）
        # print("2222")
        # print(data_loader.test_list[i])
        ...
        #得到预测卫星图像sat的GPS中心
        # print("1111")
        # print("得到预测卫星图像sat的GPS中心")
        # print(prediction_name)
        # print(prediction_name.replace('.png',''))
        # print(prediction_name.replace('.png','').split('_'))

        # print(prediction_name.replace('.png','').split('_')[1:3])
...
def meter_level_localization_from_npy(load_path=None,show=True):
    ...
    # delta_list_center = np.zeros([120, 2])
    ...
    sat_descriptor = np.load(os.path.join(load_path, 'sat_global_descriptor.npy'))
    grd_descriptor = np.load(os.path.join(load_path, 'grd_global_descriptor.npy'))
    #提取二维数组[90618,4096]中的第1到第120行
    # sat_descriptor1=sat_descriptor[0:119,:]
    # grd_descriptor1 = grd_descriptor[0:119, :]
    # similarity = np.matmul(grd_descriptor1, np.transpose(sat_descriptor1))
    similarity = np.matmul(grd_descriptor, np.transpose(sat_descriptor))
    order_list = np.argmax(similarity, axis=1)
    del(similarity)
    ...
    if show:
        ...
        plt.legend(['SAFA'], loc=2, fontsize=12)
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig("./result"+'/'+"base_line_result.png")
